...
export const ArticleModal: React.FC<ArticleModalProps> = ({ isOpen, onClose, onSave, article, currentUser }) => {
    ...
    const AiButton = ({command, label, children}: {command: 'suggest_tags' | 'improve', label: string, children?: React.ReactNode}) => (
        <button type="button" onClick={() => handleAiAssist(command)} disabled={!!isAiLoading}
                className="flex items-center justify-center gap-2 text-xs bg-slate-700 hover:bg-slate-600 text-white font-semibold py-1.5 px-3 rounded-md transition-colors disabled:opacity-50 disabled:cursor-wait">
    ...
    const TabButton = ({label, value, icon}: {label:string, value: 'write' | 'ai_draft', icon: React.ReactNode}) => (
        <button type="button" onClick={() => setActiveTab(value)}
            className={`flex items-center gap-2 px-3 py-1.5 text-sm rounded-md font-medium ${activeTab === value ? 'bg-indigo-600 text-white' : 'text-slate-300 hover:bg-slate-700'}`}>
           {icon} {label}
        </button>
    );
    ...
    return (
        <div className="fixed inset-0 bg-black/60 z-50 flex justify-center items-center p-4" onClick={onClose}>
            <div className="bg-slate-800/80 backdrop-blur-lg rounded-xl shadow-xl w-full max-w-6xl border border-slate-700 relative flex flex-col max-h-[90vh]" onClick={e => e.stopPropagation()}>
                ...
                <form onSubmit={handleSubmit} className="flex-grow overflow-hidden flex flex-col">
                    {activeTab === 'write' ? (
                        <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6 overflow-y-auto">
                            ...
                            <div className="space-y-4">
                                <div>
                                    ...
                                    <input type="text" name="title" value={formData.title} onChange={handleChange} required className="w-full bg-slate-900 border border-slate-600 rounded-lg py-2 px-3 text-white focus:outline-none focus:ring-2 focus:ring-indigo-500" />
                                </div>
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        <label htmlFor="category" className="block text-sm font-medium text-slate-300 mb-1">Category</label>
                                        <input type="text" name="category" value={formData.category} onChange={handleChange} required placeholder="e.g., Policies/HR" className="w-full bg-slate-900 border border-slate-600 rounded-lg py-2 px-3 text-white focus:outline-none focus:ring-2 focus:ring-indigo-500" />
                                    </div>
                                    <div>
                                        <label htmlFor="tags" className="block text-sm font-medium text-slate-300 mb-1">Tags (comma-separated)</label>
                                        <input type="text" name="tags" value={tagsString} onChange={e => setTagsString(e.target.value)} placeholder="hr, vacation" className="w-full bg-slate-900 border border-slate-600 rounded-lg py-2 px-3 text-white focus:outline-none focus:ring-2 focus:ring-indigo-500" />
                                    </div>
                                </div>
                                <div>
                                    <div className="flex justify-between items-center mb-1">
                                        ...
                                        <div className="flex items-center gap-2">
                                            <AiButton command="suggest_tags" label="Suggest Category & Tags"><SparklesIcon className="w-4 h-4 text-purple-400" /></AiButton>
                                            <AiButton command="improve" label="Improve Writing"><SparklesIcon className="w-4 h-4 text-purple-400" /></AiButton>
                                        </div>
                                    </div>
                                    <textarea name="content" value={formData.content} onChange={handleChange} required rows={12} className="w-full bg-slate-900 border border-slate-600 rounded-lg py-2 px-3 text-white focus:outline-none focus:ring-2 focus:ring-indigo-500 font-mono text-sm"></textarea>
                                </div>
                            </div>
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="bg-slate-900/50 p-4 rounded-lg border border-slate-700 h-full overflow-y-auto">
                                    <div className="prose prose-sm prose-invert max-w-none prose-h2:text-xl prose-h3:text-lg prose-h3:text-indigo-400 prose-headings:mb-2 prose-p:my-3 prose-ul:my-3 prose-li:my-1" dangerouslySetInnerHTML={parsedPreviewHTML} />
                                </div>
                            </div>
                        </div>
                    ) : (
                        <div className="p-6 flex flex-col items-center justify-center text-center overflow-y-auto">
                           <FilePenIcon className="w-16 h-16 text-indigo-400 mb-4" />
                           <h3 className="text-2xl font-bold text-white">Draft with AI</h3>
                           <p className="text-slate-400 mt-2 max-w-lg">Describe the article you want to create. Be as specific as possible for the best results. The AI will generate a draft which you can then edit.</p>
                           <div className="w-full max-w-lg mt-6">
                                <textarea name="ai_draft_prompt" value={aiDraftPrompt} onChange={(e) => setAiDraftPrompt(e.target.value)}
                                          ...
                                          rows={4} className="w-full bg-slate-900 border border-slate-600 rounded-lg py-2 px-3 text-white focus:outline-none focus:ring-2 focus:ring-indigo-500" />
                                <button type="button" onClick={() => handleAiAssist('draft_article')} disabled={!!isAiLoading || !aiDraftPrompt}
                                        className="mt-3 w-full flex items-center justify-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-2.5 px-4 rounded-lg transition-colors disabled:opacity-50">
                     ...
                     <div className="flex justify-end gap-3 p-4 bg-slate-800/50 border-t border-slate-700 flex-shrink-0">
                        <button type="button" onClick={onClose} className="bg-slate-600 hover:bg-slate-500 text-white font-bold py-2 px-4 rounded-lg transition-colors">Cancel</button>
                        <button type="submit" className="bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 text-white font-bold py-2 px-4 rounded-lg transition-all shadow-lg hover:shadow-indigo-500/50">{article?.id ? 'Save Changes' : 'Create Article'}</button>
                    </div>
                </form>
            </div>
        </div>
